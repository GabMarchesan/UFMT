...
direcao_pulo = 1  # Direção do pulo (subindo ou descendo)
...
def display():
    ...
    gluLookAt(camera_x, camera_y, camera_z,  # Posição da câmera
              T3_x, altura_inicial_pulo, T3_z,      # Ponto para onde a câmera está olhando (a aranha)
    ...
    # Desenha a arnha com movimento automático
    glPushMatrix()
    glTranslatef(0, 0, movimento_aranha)  # Movimenta a ovni
    glColor3f(1.0, 1.0, 1.0)  # Cor da ovni (amarelo)
...
def animacao(value):
    ...
    # Movimenta a ovni para frente e para trás
    ...
    if b'w' in keys_pressed:
        T3_x += move_speed * math.sin(angle_rad)  # Move para frente na direção x
        T3_z += move_speed * math.cos(angle_rad)  # Move para frente na direção z
...
def resize(w, h):
    ...
    if aspect_ratio > 1:
        glOrtho(-camera_distance * aspect_ratio, camera_distance * aspect_ratio, -camera_distance, camera_distance, -1000, 1000)
    else:
        glOrtho(-camera_distance, camera_distance, -camera_distance / aspect_ratio, camera_distance / aspect_ratio, -1000, 1000)
...
wind = glutCreateWindow("3D Model Viewer")
init()
