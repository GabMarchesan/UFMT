...
GRAFO* Kruskal(GRAFO* grafo, int* numArestasArvore) {
    ...
    GRAFO* arvore = criaGrafo(numVertices); // Criar um novo grafo para representar a árvore mínima
    ...
    while (arestasNaArvore < numVertices - 1 && i < numArestas) {
        ...
        if (raizOrigem != raizDestino) {
            criaAresta(arvore, origem, destino, arestas[i].peso); // Adicionar a aresta à árvore mínima
            arestasNaArvore++;
...
int main() {
    ...
    int numArestasArvore;
    // Executar o algoritmo de Kruskal para encontrar a árvore mínima
    GRAFO* arvoreMinimaKruskalNY = Kruskal(grafoNY, &numArestasArvore);

    // Calcular o custo total da árvore mínima de NY em Kruskal
    int custoArvoreKruskalNY = 0;
    for (int i = 0; i < arvoreMinimaKruskalNY->vertices; i++) {
    ...
    salvarResultado("saida_NY_Kruskal.txt", custoArvoreKruskalNY, 0.0, arvoreMinimaKruskalNY);

    // Liberar a memória da instância NY e árvore mínima
    free(grafoNY->adj);
    free(grafoNY);
