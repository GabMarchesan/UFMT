...
void salvarResultado(char* nomeArquivo, int origem, int destino, int custoArvore, double tempoExecucao) {
    ...
    fprintf(arquivo, "Custo Total da Árvore: %d\n", custoArvore);
...
int main() {
    ...
    // Calcular o custo total da árvore mínima de CAL em Kruskal
    int custoArvoreKruskalCAL = 0;
    for (int i = 0; i < grafoCAL->vertices; i++) {
        ADJACENCIA* adj = arvoreMinimaKruskalCAL->adj[i].cab;
        while (adj) {
            custoArvoreKruskalCAL += adj->peso;
            adj = adj->prox;
        }
    }
    ...
    // Calcular o custo total da árvore mínima de CAL em Prim
    int custoArvorePrimCAL = 0;
    for (int i = 0; i < grafoCAL->vertices; i++) {
        ADJACENCIA* adj = arvoreMinimaPrimCAL->adj[i].cab;
        while (adj) {
            custoArvorePrimCAL += adj->peso;
            adj = adj->prox;
        }
    }

    // Salvar os resultados em um arquivo para a instância CAL
