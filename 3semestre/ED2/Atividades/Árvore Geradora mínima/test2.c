...
void salvarResultado(const char* nomeArquivo, int origem, int destino, int custoArvore, double tempoExecucao) {
...
int calcularCustoArvore(GRAFO* arvore) {
    int custo = 0;
    for (int i = 0; i < arvore->vertices; i++) {
        ADJACENCIA* adj = arvore->adj[i].cab;
        while (adj) {
            custo += adj->peso;
            adj = adj->prox;
        }
    }
    return custo / 2; // Dividido por 2, pois as arestas são contadas duas vezes (ida e volta)
}
...
int main() {
    ...
    // Calcular o custo total da árvore mínima de NY
    int custoArvoreKruskalNY = calcularCustoArvore(arvoreMinimaKruskalNY);
    ...
    // Calcular o custo total da árvore mínima de NY
    int custoArvorePrimNY = calcularCustoArvore(arvoreMinimaPrimNY);

     // Salvar os resultados em um arquivo para a instância NY
    salvarResultado("saida_NY_Kruskal.txt", origem1, destino1, custoArvoreKruskalNY, tempoExecucaoKruskalNY);
    salvarResultado("saida_NY_Prim.txt", origem1, destino1, custoArvorePrimNY, tempoExecucaoPrimNY);
    ...
    // Repetir o processo para a instância CAL
    const char* arquivoCAL = "CAL.txt";
    GRAFO* grafoCAL = carregarInstancia(arquivoCAL);

    // Definir origens e destinos para a instância CAL
    origem1 = 0; // Escolha o vértice de origem desejado
    destino1 = 1; // Escolha o vértice de destino desejado

    origem2 = 2; // Escolha o vértice de origem desejado
    destino2 = 3; // Escolha o vértice de destino desejado

    // Medir o tempo de execução para o algoritmo de Kruskal na instância CAL
    ...
    // Calcular o custo total da árvore mínima de CAL
    int custoArvoreKruskalCAL = calcularCustoArvore(arvoreMinimaKruskalCAL);

    // Medir o tempo de execução para o algoritmo de Prim na instância CAL
    ...
    // Calcular o custo total da árvore mínima de CAL
    int custoArvorePrimCAL = calcularCustoArvore(arvoreMinimaPrimCAL);

    // Salvar os resultados em arquivos para a instância CAL
    salvarResultado("saida_CAL_Kruskal.txt", origem2, destino2, custoArvoreKruskalCAL, tempoExecucaoKruskalCAL);
    salvarResultado("saida_CAL_Prim.txt", origem2, destino2, custoArvorePrimCAL, tempoExecucaoPrimCAL);

    // Liberar a memória da instância CAL e árvores mínimas
    free(grafoCAL->adj);
    free(grafoCAL);
