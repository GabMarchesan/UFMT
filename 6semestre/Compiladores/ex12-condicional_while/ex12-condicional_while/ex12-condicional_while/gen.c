...
/**
 * @brief Gera código para uma expressão booleana, como comparação entre dois valores.
 * 
 * @param oper O operador a ser utilizado para a comparação (EQUAL, NE, LT, etc.)
 */
void gen_bool(int oper) {
    static int bool_label;
    char bool_label_name[MAX_CHAR];
    
    // Gera nome único para a label
    gen_bool_label_name(bool_label_name); 

    fprintf(output_file, "; Aplica operador booleano/expressão lógica\n");
    fprintf(output_file, "pop rbx\n");  // Pega o segundo valor da pilha
    fprintf(output_file, "pop rax\n");  // Pega o primeiro valor da pilha
    
    // Inicializa rcx com 1 (verdadeiro)
    fprintf(output_file, "mov rcx, 1\n");
    fprintf(output_file, "cmp eax, ebx\n"); // Compara os dois valores

    // Dependendo do operador, gera o salto condicional para a label
    switch (oper) {
        case EQUAL:
            fprintf(output_file, "je %s\n", bool_label_name);  // Se igual
            break;
        case NE:
            fprintf(output_file, "jne %s\n", bool_label_name);  // Se diferente
            break;
        case LT:
            fprintf(output_file, "jl %s\n", bool_label_name);  // Se menor
            break;
        case GT:
            fprintf(output_file, "jg %s\n", bool_label_name);  // Se maior
            break;
        case LE:
            fprintf(output_file, "jle %s\n", bool_label_name);  // Se menor ou igual
            break;
        case GE:
            fprintf(output_file, "jge %s\n", bool_label_name);  // Se maior ou igual
            break;
        default:
            printf("[ERRO] Operador booleano não suportado.\n");
            break;
    }

    // Caso a comparação falhe, rcx é setado para 0 (falso)
    fprintf(output_file, "mov rcx, 0\n");

    // Label para o fim da expressão lógica
    gen_label(bool_label_name);

    // Armazena o valor booleano (1 ou 0) na pilha
...
void gen_bool_label_name(char *name) {
    ...
    // Gera um nome único para a label
    sprintf(conv_value, "%d", nbool_labels);
    strcpy(str_name, "label_bool_");
    strcat(str_name, conv_value);
    strcpy(name, str_name);
    nbool_labels++;
}
...
/**
 * @brief Gera código para o comando ASSIGN
 * 
 * @param lexeme_of_id 
 * @param type 
 */
