...
void gen_data_section(void) {
    ...
    fprintf(output_file, "str0: db \"%%d\",0\n"); // Para inteiros
    fprintf(output_file, "str1: db \"%%s\",0\n"); // Para strings
    fprintf(output_file, "str2: db \"%%c\",0\n"); // Para caracteres
    fprintf(output_file, "str3: db \"%%lf\",0\n"); // Para floats
...
void gen_epilog_code(void) {
    //fprintf(output_file, "\nret\n");
    fprintf(output_file, ";encerra programa\n");
...
/**
 * @brief Funcao que gera automaticamente um novo nome para um label para a função
 * @param string name
 */
void gen_label_func_name( char *name ) {
    char str_name[MAX_CHAR];
    char conv_value[16];
    static int nlabels=0;

    sprintf(conv_value, "%d", nlabels);
    strcpy(str_name, "func_label");
    strcat(str_name, conv_value);
    strcpy(name, str_name);
    nlabels++;
}
...
/**
 * @brief Funcao que gera um jump da funcao
 * @param string label
 */
void gen_func_jump(char *label) {
    fprintf(output_file, ";jump_func\n");
...
/**
 * @brief Funcao que gera codigo para retorno de funcao
 */
void gen_return() {
    fprintf(output_file, ";retorno da funcao\n");
...
void gen_assign(char *lexeme_of_id, int type) {
    ...
    fprintf(output_file, "pop rax\n"); // Pega o resultado da expressão da pilha
    fprintf(output_file, "mov [%s], rax\n", lexeme_of_id); // Armazena em [lexeme_of_id]
}

/**
 * @brief Funcao que gera codigo de montagem para carregar o valor de uma variavel
 * 
 * @param var_name Nome da variavel
 */
void genVar(char var_name[MAX_TOKEN]) {
    fprintf(output_file, ";Carregamento de variavel\n");
    fprintf(output_file, "mov rax,[%s]\n", var_name);
    fprintf(output_file, "push rax\n");
}